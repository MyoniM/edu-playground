...
(function () {
  ...
  var grammar = {
    ...
    ParserRules: [
      ...
      {
        name: 'var_assign',
        symbols: [lexer.has('identifier') ? { type: 'identifier' } : identifier, '_', { literal: ':=' }, '_', 'expr'],
        postprocess: (data) => ({
          type: 'var_assign',
          var_name: data[0],
          value: data[4],
      ...
      {
        name: 'func_call',
        symbols: [lexer.has('identifier') ? { type: 'identifier' } : identifier, '_', { literal: '(' }, '_ml', 'func_call$ebnf$1', { literal: ')' }],
        postprocess: (data) => ({
          type: 'func_call',
          func_name: data[0],
      ...
      {
        name: 'param_list$ebnf$1',
        symbols: ['param_list$ebnf$1', 'param_list$ebnf$1$subexpression$1'],
        postprocess: function arrpush(d) {
          return d[0].concat([d[1]]);
        },
      },
      {
        name: 'param_list',
        symbols: ['param', 'param_list$ebnf$1'],
        postprocess: (data) => {
          const repeated = data[1];
      ...
      { name: 'lambda_body', symbols: ['expr'], postprocess: (data) => [data[0]] },
      { name: 'lambda_body', symbols: [{ literal: '{' }, '__lb_', 'statements', '__lb_', { literal: '}' }], postprocess: (data) => data[2] },
      ...
      {
        name: 'array',
        symbols: [{ literal: '[' }, '_', 'array$ebnf$1', { literal: ']' }],
        postprocess: (data) => ({
          type: 'array',
      ...
      {
        name: 'array_items$ebnf$1',
        symbols: ['array_items$ebnf$1', 'array_items$ebnf$1$subexpression$1'],
        postprocess: function arrpush(d) {
          return d[0].concat([d[1]]);
        },
      },
      {
        name: 'array_items',
        symbols: ['param', 'array_items$ebnf$1'],
        postprocess: (data) => {
          const repeatedPart = data[1];
          const restParams = repeatedPart.map((part) => part[1]);
          return [data[0], ...restParams];
